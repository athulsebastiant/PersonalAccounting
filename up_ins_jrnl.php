...
$insertSql = "INSERT INTO jrldetailed (EntryID, LineID, AccountID, description, DebitAmount, CreditAmount, createdBy)
              VALUES (?, ?, ?, ?, ?, ?,'$username')";
...
try {
    ...
    foreach ($insertArray as $row) {
        ...
        $insertStmt->bind_param(
            "iiisdd",  // Data types: i for int, s for string, d for double
            $entryId,                // EntryID
            $row['lineID'],          // LineID
            $row['account'],         // AccountID
            // EntityID
